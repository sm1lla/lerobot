...
def get_2D_bbox(img, prompt=None) -> str:
    ...
    response = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            system_instruction=bounding_box_system_instructions, temperature=0.5
        ),
    )
...
def get_target_bbox(img_tensor, prompt=None):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            pick, place, pick_labels, place_labels = create_pick_place_lists(
                bounding_boxes
            )
    ...
    return (
        default_pick,
        default_place,
        normalize_bbox_0to1(default_pick),
        normalize_bbox_0to1(default_place),
        default_pick_labels,
...
def get_random_targets(
    pick, place, normalized_pick, normalized_place, pick_labels=None, place_labels=None
):
    if len(pick) == 0:
        pick_target = [0, 0, 0, 0]
    ...
    return (
        pick_target,
        place_target,
        torch.tensor(normalized_pick_target),
        torch.tensor(normalized_place_target),
        pick,
        normalized_pick,
        pick_target_label,
        place_target_label,
...
def get_2D_bbox_multi_view(img_top, img_front, prompt=None) -> dict:
    ...
    front_response = client.models.generate_content(
        model=MODEL_ID,
        contents=[img_top, front_view_prompt, top_response, img_front],
...
def create_multi_view_pick_place_lists(
    top_objects, front_objects, place_location="bin"
):
    ...
    print(
        f"Found {len(place_top)} place objects and {len(pick_top)} pick objects in both views"
    )
...
def get_target_bbox_multi_view(img_top_tensor, img_front_tensor, prompt=None):
    ...
    for attempt in range(max_attempts):
        try:
            multi_view_response = get_2D_bbox_multi_view(
                img_top, img_front, prompt=prompt
            )

            top_objects = multi_view_response["top_view"]
            front_objects = multi_view_response["front_view"]

            pick_top, pick_front, place_top, place_front, pick_labels, place_labels = (
                create_multi_view_pick_place_lists(top_objects, front_objects)
            )
            ...
            if len(place_top) > 0 and len(place_front) > 0:
                ...
                return (
                    ...
                    place_front,
                    normalized_pick_top,
                    normalized_pick_front,
                    normalized_place_top,
                    normalized_place_front,
                    pick_labels,
                    place_labels,
                )

            print(
                f"Attempt {attempt+1}: No common place objects found in both views, retrying..."
            )
...
def get_random_targets_multi_view(
    pick_top,
    pick_front,
    place_top,
    place_front,
    ...
    if len(pick_top) == 0:
        ...
        normalized_place_target_top = (
            torch.tensor(normalized_place_top[0])
            if len(normalized_place_top) > 0
            else torch.tensor([0, 0, 0, 0])
        )
        normalized_place_target_front = (
            torch.tensor(normalized_place_front[0])
            if len(normalized_place_front) > 0
            else torch.tensor([0, 0, 0, 0])
        )

        pick_target_label = "none"
        place_target_label = (
            place_labels[0] if place_labels and len(place_labels) > 0 else "bin"
        )
    else:
        ...
        place_target_front = (
            place_front[place_idx] if len(place_front) > 0 else [0, 0, 0, 0]
        )
        ...
        normalized_place_target_top = (
            torch.tensor(normalized_place_top[place_idx])
            if len(normalized_place_top) > 0
            else torch.tensor([0, 0, 0, 0])
        )
        normalized_place_target_front = (
            torch.tensor(normalized_place_front[place_idx])
            if len(normalized_place_front) > 0
            else torch.tensor([0, 0, 0, 0])
        )
        ...
        pick_target_label = pick_labels[pick_idx] if pick_labels else "object"
        place_target_label = (
            place_labels[place_idx] if place_labels and len(place_labels) > 0 else "bin"
        )
        ...
        normalized_pick_top = [
            p for i, p in enumerate(normalized_pick_top) if i != pick_idx
        ]
        normalized_pick_front = [
            p for i, p in enumerate(normalized_pick_front) if i != pick_idx
        ]
    ...
    return (
        ...
        place_target_top,
        place_target_front,
        normalized_pick_target_top,
        normalized_pick_target_front,
        normalized_place_target_top,
        normalized_place_target_front,
        pick_top,
        pick_front,
